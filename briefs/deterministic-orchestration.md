...
`/kickoff` is a state machine written in prose. The orchestrating model reads `.claude/skills/kickoff/SKILL.md` and *performs* the machine: resolve venue (Step 0a), resolve phase and lane (Step 1), decompose (Step 1a), delegate to four roles, parse verdicts by string match, enforce bounded revision loops (2 plan cycles, 2 code cycles, 3 build-fix cycles), run the cross-harness fallback state machine, run the ripple pass, assemble the END block.
