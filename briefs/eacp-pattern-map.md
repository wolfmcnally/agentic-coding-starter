...
| Pattern | Where it lives | Notes |
|---|---|---|
| [Feedback Sensor](https://aipatternbook.com/feedback-sensor) | `bin/check`; verdicts | Both classes present: one repository-owned machine-oracle entry point (locked lint/format/tests/policy checks) and the two judgment critics. |
...
| [Architecture Fitness Function](https://aipatternbook.com/architecture-fitness-function) | `bin/check policy`; `tests/test_check.py`; `tests/test_kickoff_config.py` | The canonical policy mode enforces public-repo and configuration invariants; behavioral tests prove the gate wrapper itself cannot hide prerequisite or child-command failures. **Partial** — catalog/phase-ledger structure is not yet mechanized. |
...
1. **[Architecture Fitness Function](https://aipatternbook.com/architecture-fitness-function) — still partial against the repo's own law.** `briefs/BRIEF.md` §7 lists further mechanizable criteria: every `briefs/` file appears in the `CLAUDE.md` catalog and vice versa, the same for `policies/`, and `plan/INDEX.md` carries exactly one `⬅️`. `bin/check` is now the canonical home for these checks, but the catalog/ledger assertions themselves remain to be encoded.
...
3. **[Skill Fitness](https://aipatternbook.com/skill-fitness) — absent.** Six skills exist; none is versioned, scoped by measured lift, or subject to a retirement test.
...
8. **[Reflexion](https://aipatternbook.com/reflexion) — absent as a mechanism.** Revision rounds pass the critic's findings forward but never ask the coder to articulate *why* the last attempt failed. `LOG.md` records outcomes, not failure analyses.
