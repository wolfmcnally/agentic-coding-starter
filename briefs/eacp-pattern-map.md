...
1. **[Architecture Fitness Function](https://aipatternbook.com/architecture-fitness-function) — closed 2026-08-10.** The catalog/ledger assertions `briefs/BRIEF.md` §7 specified are now encoded: `bin/check-catalogs` (registered as the `policy-catalogs` gate in `bin/check`, covered by `tests/test_check_catalogs.py`) enforces briefs/policies catalog sync in both directions and the single-`⬅️` invariant; `bin/lessons validate` (`policy-lessons` gate) enforces the ledger schema.
