...
- **Retrieval caveat (measured, 2026-07-23):** the corpus's `search` tool was unreliable during this work — roughly **one call in three hard-hangs and is cut off by the CDN at exactly 30 s** (CloudFront origin-response timeout). A controlled run gave `search` 6/10 successes against 20/20 for `list_articles` and `fetch_article` on the same host and session path, isolating the fault to the semantic-retrieval half rather than the corpus or the network. The map below was therefore built from **deterministic manifest enumeration plus targeted `fetch_article` calls**, not semantic search. Consequence for this brief: coverage of the *named* corpus was complete **as of that retrieval**, but a pattern whose relevance would only surface through a semantic match — rather than through its title or section — could have been missed.
- **Currency (checked 2026-08-25):** the corpus has since grown to **316 articles, 313 draft**, across the same 14 sections — 21 entries this map has never seen. Its coverage claim is therefore scoped to the 2026-07-23 snapshot above, not to the corpus as it stands. A spot check of the two most repo-relevant sections (Agentic Software Construction, 67 articles; Agent Governance and Feedback, 30) found **28 of those 97 absent from this map by both slug and title** — a mechanical lead rather than a verdict, since a selective map is entitled to omit what does not apply, and several (`graph-rag`, `react`, `prompt-caching`) plainly do not. Others land on shipped machinery and look like real gaps: `evaluation-gate`, `steering-loop`, `loop-engineering`, `plan-and-execute`, `agent-workflow-graph`, `trusted-monitoring`, the `premature-termination` antipattern, and `incident-to-eval-synthesis` (already cited in [`harness-self-improvement.md`](harness-self-improvement.md) §4, but absent here). Re-running the map against the current corpus is unscheduled work, not a claim this brief makes.
- **Method:** section manifests were enumerated in full for the eight sections plausibly relevant to an agentic-methodology repo; the six highest-stakes entries (`orchestrator-workers`, `generator-evaluator`, `prompt-chaining`, `externalized-state`, `compound-engineering`, `harness-engineering`) were read in full before any claim was made about them. The remainder are matched against their canonical one-line summaries.
...
|---|---|---|
| [Prompt Chaining](https://aipatternbook.com/prompt-chaining) | `.claude/skills/kickoff/SKILL.md` and its directly linked stage resources | **This, not Orchestrator-Workers, is the correct name for `kickoff`.** The corpus's test is whether the steps are fixed in advance or invented per request. `kickoff`'s are fixed: resolve → preflight → identify → plan → plan-review → code → code-review → build → accept → log. The corpus's own worked example ("criteria before code, code before tests, tests before merge") is the same shape. |
...
| — independent context, intensified | `briefs/cross-agent-invocation.md` §§1, 4 | The corpus asks for independent context windows. The evaluator runs in an independent role context (quality/same-harness by default, cross-vendor explicitly selectable through the role policy), and the handoff **redacts the implementer's self-assessment** — no Build Status block, no "tests pass" narrative — on the cited finding that cold artifacts yield ~9.4 mean review findings vs 2.4–4.0 with the implementer's framing attached. |
...
| [Orchestrator-Workers](https://aipatternbook.com/orchestrator-workers) | `.claude/skills/kickoff/preflight.md` Step 1a; `.claude/skills/kickoff/close.md` Step 9a | **Present only in the decomposition move.** The pipeline is a chain, but authorized child decomposition chooses subtasks at consequential boundaries, one at a time, with each predecessor’s outcomes in hand. A coherent major phase may need no children. That is the orchestrator half; the rest is chain. |
...
| [Model Routing](https://aipatternbook.com/model-routing) | `kickoff.yaml` `role_models`; `policies/role-models.md`; `.claude/skills/roles/SKILL.md` | Harness-aware: `role_models[H][role]` → `role_models['default'][role]` → native. Quality, balanced and economy presets balance model capability and cost through ordinary pins; quality/same-harness is the default, while cross-vendor review is an explicit option whose incremental value remains to be measured. |
| [Reasoning Effort](https://aipatternbook.com/reasoning-effort) | `kickoff.yaml` `role_models.*.effort` | A separate field from `model`; all shipped preset pins begin at high effort, with selector-specific supported settings defined in the role policy. |
...
| [Checkpoint](https://aipatternbook.com/checkpoint) | `.claude/skills/kickoff/preflight.md` Step 0b preflight; the three-signal gate in `implementation.md`; Steps 7–8 in `acceptance.md` | Four pause-verify-proceed points. The preflight is the strongest: it aborts *before any phase state is mutated*. |
| [Verification Loop](https://aipatternbook.com/verification-loop) | `.claude/skills/kickoff/acceptance.md` Step 7’s classify-and-route fix loop | Failures are classified (coder error / plan error / environment error) and routed to the role that can fix them, rather than thrown back undifferentiated. |
...
| [Task Horizon](https://aipatternbook.com/task-horizon) | `briefs/methodology.md` §6; `.claude/skills/kickoff/preflight.md` Step 1a | A demonstrated coherence limit can justify a boundary. Coherent outcomes stay intact across surfaces; session length, model reputation and first-cycle approval are not capability measurements or automatic sizing rules. |
...
| [Skill](https://aipatternbook.com/skill) | `.claude/skills/{kickoff,methodology,rule-one,learn,teach,roles,sweep,sweep-planning,sweep-coding,demo,treatise,plain,ask,stamp}/SKILL.md` | Fourteen packaged workflows. `rule-one` supplies the diagnostic discipline before learning is persisted; `roles` is a thin intelligence wrapper over a deterministic script; `sweep` is the user-gated maintenance half of the lessons flywheel. |
...
| [Shift-Left Feedback](https://aipatternbook.com/shift-left-feedback) | Plan review before code exists; `.claude/skills/kickoff/preflight.md` Step 0b preflight before phase state exists | Both are the pattern's shape: move the check as close to creation as possible. The preflight is the sharper one — it validates the *upstream infrastructure* before a single marker flips. |
| [Fail Fast and Loud](https://aipatternbook.com/fail-fast-and-loud) | `.claude/skills/kickoff/preflight.md` Step 0b: "Any failure aborts `kickoff` immediately" | And explicitly refuses the soft landing: "Do not fall back to native, identify or decompose the phase, change `plan/INDEX.md`, append a START/END block, or invoke an agent." |
...
| [Agent Provenance](https://aipatternbook.com/agent-provenance) | `.claude/skills/kickoff/close.md` Step 10 END block | Requested model, effort and venue are reported separately from qualified provider observations, with missing observations explicitly unreported. Failed dispatch evidence survives governed recovery; selected model and effort cannot silently change. Any authorized venue difference carries its reason and authority. |
...
| [Thread-per-Task](https://aipatternbook.com/thread-per-task) | `kickoff/SKILL.md`: single-phase invocation | One coherent outcome per invocation; truthful parks and fresh runs preserve continuity when authority changes. |
| [Compound Engineering](https://aipatternbook.com/compound-engineering) | `CLAUDE.md` invariants: **Rules, not memory; Rule One** and the lessons-harvest obligation; `.claude/skills/kickoff/close.md` Step 9c; `policies/lessons.md` | The corpus asks that every closed unit of work codify its lesson onto a durable surface. This repo makes surface *selection* a hard invariant ("capture it in the repo, not in memory") and, as of 2026-08-10, makes the closing condition **mechanical**: `.claude/skills/kickoff/close.md` Step 9c asks the lessons question at every phase close, the END block's `Lessons:` field may not be omitted, and candidate lessons land in the `lessons/` ledger scope-classified for graduation. Of the corpus's five canonical surfaces, the repo uses four — instruction file, skills, subagents, tests. It still declines hooks as a codification surface (§9); deterministic enforcement routes to `bin/` gates instead. |
...
| [Question Generation](https://aipatternbook.com/question-generation) | `stamp`; `plan-reviewer`'s `AskUserQuestion`; `.claude/skills/ask/SKILL.md` | `stamp` interviews only when the description leaves an answer non-obvious. The plan reviewer is the sole role permitted to escalate a product decision to the human — and when it runs in an external venue where `AskUserQuestion` is unreachable, the unresolved question becomes a `REVISE` verdict stating the question. Graceful degradation of an escalation channel. `ask` is the pull direction: operator-invoked only, it inventories every open loop the agent is holding and asks them batched, recommendation first, so the human learns of five decisions from the first message rather than the fifth. |
...
|---|---|---|
| [Worktree Isolation](https://aipatternbook.com/worktree-isolation) | `.claude/skills/kickoff/implementation.md` Step 5 item 3 | The corpus's problem — two writers on one tree — is solved by **serialization instead**: "`kickoff` is sequential, so during this stage no native writer touches the tree." The single-writer guarantee is obtained without the worktree's setup cost and cleanup risk. |
...
5. **Fail-closed venue preflight** (`.claude/skills/kickoff/preflight.md` Step 0b). One live real-read call per unique `(CLI, model, effort, access mode)` target, in an isolated temp directory, using production credential scrubs and flags. The venue must read and return unpredictable ASCII text from a local file; the manager validates the response and computes the file SHA-256, and the resulting receipt binds the routing configuration and complete target set — because CLI presence, echoed prompt text, auth-status output, and credential files do not prove that read access, precedence, entitlement, network, and the headless invocation all work together. A named pattern for *proving a delegation target is live before mutating any state* is missing from the corpus.
...
8. **[Reflexion](https://aipatternbook.com/reflexion) — closed 2026-08-10.** Every revision round now requires the coder's failure analysis — *why* the previous attempt produced the findings, root cause not restatement — enforced fail-closed by `bin/kickoff-evidence capture-change`, carried in the revision packet's `## Failure analysis` section for the next review, and read by `.claude/skills/kickoff/close.md` Step 9c as lessons-harvest sensor input.
...
- Encyclopedia of Agentic Coding Patterns, Wolf McNally, `https://aipatternbook.com` — retrieved 2026-07-23 via the `eacp` MCP server (295 articles, 292 draft; no pinned corpus revision available; corpus stood at 316/313 when last checked 2026-08-25). Individual entries cited inline.
