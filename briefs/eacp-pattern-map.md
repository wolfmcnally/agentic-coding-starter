...
| [Skill](https://aipatternbook.com/skill) | `.claude/skills/{kickoff,methodology,rule-one,learn,teach,roles,sweep,sweep-planning,sweep-coding,demo,treatise,plain,stamp}/SKILL.md` | Thirteen packaged workflows. `rule-one` supplies the diagnostic discipline before learning is persisted; `roles` is a thin intelligence wrapper over a deterministic script; `sweep` is the user-gated maintenance half of the lessons flywheel. |
