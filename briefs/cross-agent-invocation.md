...
This brief pins the researched best current practices (as of mid-2026) for invoking OpenAI's `codex` CLI from inside Claude Code and Anthropic's `claude` CLI from inside Codex, so future work cites a stable position instead of re-deriving it. It also records the design rationale for **cross-harness review** — an explicit option of the per-role model/venue feature, which pins a separate model and effort per canonical role, scoped by which harness is orchestrating.
...
- **Revision rounds: `codex exec resume <session-id> ...`** preserves the reviewer's context across rounds. **The `resume` subcommand has a different flag surface than `exec` — `-s/--sandbox` and `-C/--cd` do not exist on it** (codex-cli 0.136.0 rejects `-s` with `error: unexpected argument '-s' found`, exit 2 — empirically verified). Set the sandbox through a config override instead, and `cd` into the repo rather than passing `-C` (resume filters recorded sessions by cwd). `--model` and `-c model_reasoning_effort=...` are accepted on resume (re-verified with codex-cli 0.144.0) and must be repeated for an explicit pin:
  ...
  A naive resume that copies the original `exec` flags fails flag parsing. Preserve that failed evidence and park unless an already-authorized recovery preserves selected model, effort and required authority; any authorized fresh call carries full updated context and the same model, effort and required authority.
...
- **`--model <alias|id>` selects the model.** Omit it only for a provider-default pin. Explicit `opus` and `fable` pins use their corresponding aliases through the manager. An unrecognized alias fails closed; it does not authorize substituting another identifier.
...
- **Live-preflight every non-native target before mutating phase state.** Resolve the phase's external roles first, group them by unique `(CLI, model, effort, access mode)`, and make one minimal real-read call per group using the production auth scrubs, flags, stdin closure, and read-only/write-enabled posture. In an isolated temporary directory, create unpredictable ASCII text and require the venue to read and return it exactly. Keep the text out of the prompt. The manager validates the response and computes the file SHA-256 for the receipt; the model needs only its declared read tools. Codex needs `--skip-git-repo-check` on this probe alone; the production checkout invocation does not. Persist the result with `./bin/kickoff-config preflight --receipt <path>`: the receipt binds the routing-config digest, harness, target descriptors, and probe digest, including the all-native empty-target case. A known echoed sentinel, CLI presence, auth-status output, and credential files do not prove read access, precedence, entitlement, network, and the headless invocation together. Any preflight failure aborts before status/log mutation rather than silently weakening the configured role topology.
...
These BCPs are applied by the per-role model/venue feature and the per-role execution budgets. Their separate model and effort fields plus first-event, idle-progress, and hard-deadline budgets live together in human-editable `kickoff.yaml`. Its shipped default is quality/same-harness; balanced/economy presets reduce coder or all-role cost, and cross-vendor review is explicitly selectable through ordinary role pins. Independent review contexts remain required. Requested routing and qualified provider observations are reported separately; missing observations remain unreported. Before phase state begins, `./bin/kickoff-config preflight --receipt <run-path>` live-validates every target and records the configuration-bound real-read proof. Production external calls run through `watch`, which generates and supervises the child command from resolved metadata; `render-command` exposes the same generated argv for inspection. Calls read the same canonical role file as native subagents, stream progress into the shared exact execution trace, and register immutable evidence. Orchestration and build gates are never delegated.
