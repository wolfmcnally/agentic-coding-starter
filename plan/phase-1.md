...
The right tool is the `stamp` skill, not `kickoff`.
...
The `stamp` skill will create the new directory, transfer the template's structural files, customize them for your project, and leave you with a project ready for `kickoff` from a real Phase 1. You can return here to this repo afterward and either close out Phase 1 (`✅ Completed` — "adoption happened via stamp") or leave it `⬅️ Next` for the next user.
...
5. **Run `/kickoff` in Claude Code or `$kickoff` in Codex** to start the real Phase 1.
...
- Manual check: after changing into the new project, `/kickoff` in Claude Code or `$kickoff` in Codex picks up its real Phase 1 (not this placeholder).
...
- The starter's gates still pass (`cd project && uv run ruff check example tests ../bin/kickoff-config ../tests && uv run ruff format --check example tests ../bin/kickoff-config ../tests && uv run pytest -q tests ../tests`), unless the user has replaced the example deliverable with a different surface — in which case its gates plus the root methodology tests pass.
