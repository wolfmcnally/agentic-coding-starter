...
- [`execution-telemetry.md`](policies/execution-telemetry.md) — requires one exact shared execution trace for stages, roles, waits, tools, and gates; defines union-based timing metrics (with review-convergence integers counted over the whole merged ledger, and the derived-metrics overlay as the only post-finalization stand-in), fail-closed joins, recovery, privacy projection, and the deterministic offline end-of-phase HTML report.
