...
- [`role-models.md`](policies/role-models.md) — harness-aware per-role model/venue. The two-level `role-models.yaml` (set via `/roles`) resolves each of the four roles to a model/harness, scoped by which harness orchestrates; `/kickoff` invokes any non-native role via the cross-harness recipes (write-enabled for the coder), resuming its session across rounds. The shipped default gives cross-vendor review (reviewer + critic in the other harness); orchestration and build gates are never pinnable. Owns the fallback state machine (three-signal gate, timeouts, per-stage fallback) and the END-block contract. A reachable-but-failed call falls back to native with a 🚨; an absent CLI falls back quietly.
