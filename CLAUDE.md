...
- [`methodology-treatise.md`](briefs/methodology-treatise.md) — the canonical outward explanation of this repository for practitioners and engineering leads: the thesis, the six governing principles, the architecture, the phase workflow, the consequential decisions and their tradeoffs (including the typed acceptance boundary and autonomous delivery), the two-tier improvement flywheel, the honest limits, and a verification table. Source of truth for every derivative rendering; corrections land here first per [`policies/treatise.md`](policies/treatise.md).
