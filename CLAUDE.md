...
- [`role-timeouts.md`](policies/role-timeouts.md) — first-event, idle-progress, and hard deadlines under `kickoff.yaml`'s `role_timeouts`. The unified manager enforces external calls, records gitignored telemetry, and recommends evidence-based recalibration without rewriting values automatically. Every budget is bounded by the orchestrating harness's foreground command ceiling, which silently clamps rather than refusing, so roles dispatch through tracked background work and an interrupted dispatch is diagnosed at the caller before the venue.
