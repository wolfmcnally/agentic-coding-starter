...
Which model runs each of the four `/kickoff` roles is set in `role-models.yaml` (via the `/roles` skill), scoped by which harness orchestrates; the model code name implies its CLI. GPT-5.6 uses the stable pins `sol`, `terra`, and `luna`; reasoning effort can be suffixed on Codex or Claude models (`sol@medium`, `opus@high`, `fable@max`). The shipped default routes **reviewer + critic to the *other* harness** (cross-vendor review — `codex` when Claude Code orchestrates, `claude` when Codex does) and leaves planner + coder native. There is no separate on/off switch — edit the config to change venues. Governed by [`policies/role-models.md`](policies/role-models.md); invocation recipes in [`briefs/cross-agent-invocation.md`](briefs/cross-agent-invocation.md).
...
- [`role-models.md`](policies/role-models.md) — harness-aware per-role model/venue. The two-level `role-models.yaml` (set via `/roles`) resolves each of the four roles to a model, its implied CLI, and optional reasoning effort, scoped by which harness orchestrates; `/kickoff` invokes any non-native role via the cross-harness recipes (write-enabled for the coder), resuming its session across rounds with the same model/effort. GPT-5.6 pins are `sol`, `terra`, and `luna`; Claude pins accept the same effort suffix plus `@max`. The shipped default gives cross-vendor review (reviewer + critic in the other harness); orchestration and build gates are never pinnable. Owns the fallback state machine (three-signal gate, timeouts, per-stage fallback) and the END-block contract. A reachable-but-failed call falls back to native with a 🚨; an absent CLI falls back quietly.
...
- `.claude/skills/` — slash-command surface for Claude Code.
  ...
  - `roles/SKILL.md` — pins a model (whose code name implies its CLI) and optional reasoning effort to any of the four canonical roles (thin wrapper over `bin/role-models`); governed by [`policies/role-models.md`](policies/role-models.md).
...
- **Role-model pinning / cross-harness invocation.** Choosing which model runs each of the four canonical roles via the two-level `role-models.yaml` (set with `/roles`, read by `/kickoff` at Step 0a), scoped by which harness orchestrates. The model code name implies its CLI; GPT-5.6 pins are `sol`, `terra`, and `luna`, and both Codex and Claude models accept optional reasoning effort (`sol@medium`, `opus@high`, `fable@max`). The shipped default runs reviewer + critic in the *other* harness (cross-vendor review — `codex` from Claude Code, `claude` from Codex); any role can be routed anywhere, the coder write-enabled. Orchestration and build gates always run on the session model. A reachable-but-failed delegated call falls back to native with a 🚨; an absent CLI falls back quietly. Governed by `policies/role-models.md`; recipes in `briefs/cross-agent-invocation.md`.
