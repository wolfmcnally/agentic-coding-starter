...
- After final critique, command zero and the complete implementation-candidate sequence end with `./bin/check all` against unchanged bytes. Validate and finalize evidence; materialize accepted close before captured status changes. Validate and bind prospective child completion; parent completion requires separate acceptance.
