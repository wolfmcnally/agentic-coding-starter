...
Resolve once per session, before phase work begins, per [`policies/role-models.md`](../../../policies/role-models.md). This resolves a `(venue, model)` for **each of the four roles** from the harness-aware `role-models.yaml`.
...
3. **Read + resolve.** Run `./bin/role-models --show` — it reads `role-models.yaml` and prints the resolved value per role for the current harness. (The resolution rule is: `config[H][role]` if set, else `config['default'][role]` if set, else `default`.) Each role resolves to one of `default | claude | codex | opus | fable`.
...
Remember each role's resolved `(venue, model)` and the orchestrating harness for Steps 3–6 and the Step 10 END block. Roles do not re-resolve mid-session. A non-`default` value always goes through the CLI recipe — do **not** short-circuit "value == session model" (uniform resolution, no session-model probing).
...
**Delegated venue** (`claude`/`codex`/`opus`/`fable`, per Step 0a): run the planner in that CLI per [`policies/role-models.md`](../../../policies/role-models.md), reusing the read-only recipe in [`briefs/cross-agent-invocation.md`](../../../briefs/cross-agent-invocation.md) with the model flag added (`claude --model <m>` / `codex` with no `-m`) and `KICKOFF_DELEGATION_DEPTH=1` in the child environment. The planner is read-only but needs its full tool stance — `--allowedTools "Read,Grep,Glob,WebSearch,WebFetch"` for claude; codex read-only sandbox. Instruct the external agent to read `.claude/agents/phase-planner.md` and adopt that role; pass the phase reference and full phase text via a temp file. Capture the session id for revision rounds (codex `--json` `thread_id`; claude `.session_id`) — the planner resumes across plan-revision rounds. On failure, fall back to the native `phase-planner` and record it for Step 10 — a *reachable* CLI whose call failed (three-signal gate, timeout) is a 🚨 disconnect; a CLI simply *not on PATH* is a quiet note.
...
**Delegated venue** (the value `reviewer` resolved to in Step 0a — `claude`/`codex`/`opus`/`fable`): run the role in that CLI per [`policies/role-models.md`](../../../policies/role-models.md). The default `role-models.yaml` resolves reviewer to the *other* harness (cross-vendor review); a project may resolve it anywhere. Add the model flag (`claude --model <m>` / `codex` with no `-m`) to the recipe below; everything else is identical. A reachable-but-failed call falls back to native with a 🚨 in Step 10; a CLI not on PATH falls back with a quiet note.
...
**Delegated venue** (the value `critic` resolved to in Step 0a — `claude`/`codex`/`opus`/`fable`): run the role in that CLI per [`policies/role-models.md`](../../../policies/role-models.md). The default `role-models.yaml` resolves critic to the *other* harness (cross-vendor review). Add the model flag (`claude --model <m>` / `codex` with no `-m`); a reachable-but-failed call falls back with a 🚨 in Step 10, a CLI not on PATH with a quiet note.
...
- Planner: native (<session model>) | claude | codex | opus | fable <annotate "native (<cli> not on PATH)" or "[fallback: <reason>]">
- Reviewer (plan review): native | claude | codex | opus | fable | skipped (light lane) <same annotations>
- Coder: native (<session model>) | claude | codex | opus | fable <same annotations>
- Critic (code review): native | claude | codex | opus | fable <same annotations>
...
- Per-role model/venue ([`policies/role-models.md`](../../../policies/role-models.md)): the harness-aware `role-models.yaml` (set via `/roles`) resolves each of the four roles to a model/venue at Step 0a, scoped by which harness is orchestrating. The shipped default routes reviewer + critic to the *other* harness (cross-vendor review — there is no separate on/off token) and leaves planner + coder native; a project may resolve any role anywhere. A role resolving to a CLI is invoked there (write-enabled for the coder), resuming the same session across the role's rounds. Orchestration and build gates always run on the session model — never pinnable. Fallback is graceful, per-stage, reported in the END block, and never a phase failure: a *reachable* CLI whose call failed surfaces a 🚨 in the Step 10 report; a CLI simply *not on PATH* is a quiet note. The recursion guard env var is `KICKOFF_DELEGATION_DEPTH` (a delegated role never re-delegates). Recipes and handoff hygiene: [`briefs/cross-agent-invocation.md`](../../../briefs/cross-agent-invocation.md).
