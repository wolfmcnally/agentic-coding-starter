...
When the approved work changes governing prose, perform the approved preparation first, retain its exact reports and candidate lineage, and close that run truthfully parked. Capture the final authorities and already-in-progress marker in a fresh run before genuine remaining executable work. Read [planned continuation](recovery.md#planned-continuation): eligible work carries its independently approved plan through the existing full-cycle route; otherwise run fresh planning and independent plan review. Do not rewrite stored hashes, waive authority checks, manufacture edits, or claim preparation closes the phase. Later governing changes require another truthful park/rebind.
