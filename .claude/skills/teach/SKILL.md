...
- **Out of scope** — donor-specific (starter-specific meta-skills like `/starter`, the template's `example/` project, any policy explicitly marked starter-only — currently `policies/anonymize-log-references.md` — and its enforcement tooling `scripts/check-anonymization.sh` plus `scripts/anonymization-denylist.local.example`). Do not transfer.
...
- **Tier 2 — Universal template content.** Every file under `policies/` (including the recently-added `phase-ripple.md`, which codifies the AUTO/DECIDE ripple contract `/kickoff` Steps 9a/9b implement, and `cross-harness-review.md`, which config-gates delegation of `/kickoff`'s review stages to the other harness's CLI), `methodology.md`, `agentic-bootstrap.md`, `cross-agent-invocation.md` (the cross-CLI invocation BCPs the cross-harness-review policy cites), the `AGENTS.md` symlink convention, the `.codex/prompts/<name>.md` file-symlink convention, and the `.agents/skills/<name>` directory-symlink convention (Codex CLI's native skill-discovery path, per [developers.openai.com/codex/skills](https://developers.openai.com/codex/skills); directory-symlink shape because of [openai/codex#11314](https://github.com/openai/codex/issues/11314)).
