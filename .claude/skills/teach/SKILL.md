...
- **Tier 2 — Universal template content.** Every file under `policies/` (including the recently-added `phase-ripple.md`, which codifies the AUTO/DECIDE ripple contract `/kickoff` Steps 9a/9b implement, `role-models.md`, which governs the harness-aware per-role model/venue resolution — including the shipped cross-vendor-review default, and `review-lanes.md`, which lets a phase declare `light` review for mechanical work), `methodology.md`, `agentic-bootstrap.md`, `cross-agent-invocation.md` (the cross-CLI invocation BCPs the role-models policy cites), `deterministic-orchestration.md` (draft universal brief: decision criteria for a deterministic kickoff loop), the `AGENTS.md` symlink convention, the `.codex/prompts/<name>.md` file-symlink convention, and the `.agents/skills/<name>` directory-symlink convention (Codex CLI's native skill-discovery path, per [developers.openai.com/codex/skills](https://developers.openai.com/codex/skills); directory-symlink shape because of [openai/codex#11314](https://github.com/openai/codex/issues/11314)).
...
- **Role-models adoption.** Does the target's `/kickoff` SKILL.md have the Step 0a harness-aware per-role venue resolution and the delegated-role branches? Does it hold `policies/role-models.md`, `bin/role-models`, the `/roles` skill, and `briefs/cross-agent-invocation.md`? Porting the skill/policy/script/brief machinery is mechanical — AUTO. The `role-models.yaml` *values* are the target owner's choice: seed the cross-vendor-review default (via `bin/role-models --reset`) and name it in the LOG entry so the owner knows `/roles` changes it.
