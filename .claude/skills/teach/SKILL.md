...
- **Out of scope** — donor-specific (starter-specific meta-skills like `/starter`, the template's `example/` project, any policy explicitly marked starter-only — currently `policies/anonymize-log-references.md` — and its enforcement tooling `bin/check-anonymization.sh` plus `bin/anonymization-denylist.local.example`). Do not transfer.
