...
- The new `CLAUDE.md` catalogs reference every brief, policy, skill and canonical role. Verify its four zone markers occur once in order, hard rules 1/2 and waiver wording survive, root size is at most 16384 UTF-8 bytes, and kickoff entry size is at most 8192. Shorten repetition or move explanation to its owning brief/policy; never delete an essential obligation to fit. Confirm the local Astra brief and template docs pins did not transfer.
