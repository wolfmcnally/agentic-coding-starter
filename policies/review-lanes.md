...
Write the approved scope down before the work starts, as a `METHODOLOGY SCOPE` block in `LOG.md` under [log-discipline.md](log-discipline.md), and hand the independent reviewer that record rather than a description composed after the fact. This is the route's substitute for the phase file the four-role loop gives its critic: without it the review can only measure the work against the implementing agent's own account of the work, so scope drift the implementer did not notice is invisible to the one check meant to catch it. The record names the approved outcome, the surfaces in scope and anything explicitly excluded; it is written once and not revised to match what was built. Operator-ratified 2026-09-05, after the route's first run left its reviewer reconstructing the approved outcome from the dispatch prompt.

Independent review uses a separate context and examines the complete implementation against that recorded scope and applicable rules. The invoking agent runs focused checks first, addresses all required findings directly and obtains review of those corrections as needed. “One independent review” means one initial comprehensive pass, not permission to ship unresolved findings or forbid necessary verification of a fix. Reopen planning only for a consequential scope or design decision; do not regenerate an unchanged plan to carry a correction.

Preserve both full gates: after independent approval, run the required implementation-candidate sequence ending in `./bin/check all`; after all log, status, lesson and report writes, run the second bare `./bin/check all`, with no tracked write afterward. At close, record the actual changed paths, independent verdict and resolutions, exact reviewed candidate, gate commands/results and remaining human criteria against the scope record written at start; a close whose delivered scope departs from that record states the departure rather than restating the scope. Existing hard rules, delivery authority, test governance and human acceptance remain binding. Do not fabricate phase-role attempts, command receipts or an accepted `kickoff` run for direct work. When changing routes during an existing run, close that run truthfully, preserve its artifacts and completed work, and verify the complete resulting change under the newly approved route.
