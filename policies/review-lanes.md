...
Operator-ratified 2026-09-07. All authorized work on the methodology itself is implemented one-shot by the invoking primary. No delegated planner or coder, independent plan review, code critique, or phase-role loop is required or invoked. This applies across harnesses and capability tiers, in this template and derived repositories. It includes policies, briefs, skills, role definitions, configuration, workflow/evidence tooling, associated tests, and methodology knowledge transfer through `teach` and `learn`.

One-shot means one primary owns assessment, planning where useful, adaptation, implementation, self-inspection, corrections, tests, and delivery. It does not prohibit iteration after a failed test or require a single response. It is not the product lane named `one-shot`. Methodology work never creates product phase rows; ordinary product work cannot borrow this route by relabeling itself.

The primary has standing commit and fast-forward-push authority in each repository affected by authorized methodology work or transfer, after required checks and both full gates pass. No separate commit/push approval is required. Stage attributable paths only, inspect staged changes, verify delivery, and respect explicit operator restrictions. This grants no remote selection, destructive git operation, history rewrite, unrelated scope, or human acceptance. Existing staging, hook-refusal, divergence, and custody rules remain binding.

Before implementation, record the authorized outcome, surfaces and exclusions once through `bin/log-append` as `METHODOLOGY SCOPE`. Do not rewrite it to match the result. The primary self-checks the complete change against that record and applicable requirements. Run focused checks and the complete implementation-candidate sequence ending in `./bin/check all`. After all tracked bookkeeping, run the second bare `./bin/check all`; no tracked write follows its success.

At close record actual paths, scope departures, primary self-check, exact candidate, gates and remaining human criteria. Record independent review as `not applicable — primary one-shot methodology work`. Do not fabricate role reports, reviewer approval, or an accepted kickoff run for this direct route. `teach` and `learn` retain recipient adaptation, privacy and tests, but cannot add independent review or repeated ratification of already-authorized scope. A genuine new consequential decision still belongs to the operator.
