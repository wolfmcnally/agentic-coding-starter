...
For the design task, an independent evaluator reads the planner's question and any preceding writes before supplying the fixed ruling in its private rubric. Record whether the decision was surfaced before implementation; final code passing cannot repair a failed process criterion. The ruling becomes authorized public input only at that point. For every task, answer only factual clarifications, preserve each answer in the run record, and give equivalent questions equivalent answers. An unresolved consequential choice outside the rubric parks the cell instead of inventing a favorable criterion.
